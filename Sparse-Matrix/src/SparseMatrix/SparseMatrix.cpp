...
template<typename T>
SparseMatrix<T> SparseMatrix<T>::multiply(const T & x) const
{
...
template<typename T>
SparseMatrix<T> SparseMatrix<T>::operator * (const T & x) const
{
...
SparseMatrix<T> SparseMatrix<T>::transpose() const
{
	...
	SparseMatrix<T> result(this->n, this->m);
	for (int i = 1; i <= this->m; i++) {
		for (int j = 1; j <= this->n; j++) {
